...
@interface BannerViewController () <GNAdViewDelegate>
...
- (void)viewDidLoad {
    ...
    _zoneIDText = [[UITextView alloc] init];
    _zoneIDText.text = @"Zone ID";
    _zoneIDText.frame = CGRectMake(50, 80, 60, 30);
    ...
    _showAdButton = [UIButton buttonWithType:UIButtonTypeSystem];
    [_showAdButton setTitle:@"Show Ad" forState:UIControlStateNormal];
    _showAdButton.frame = CGRectMake(self.view.center.x - 125, 130, 250, 50);
    [_showAdButton addTarget:self action:@selector(showAdButtonPushed:) forControlEvents:UIControlEventTouchUpInside];
    [_scrollView addSubview:_showAdButton];
...
- (void)showAd {
    ...
    CGFloat yOffset = 200; // Starting Y position to place the first ad
    for (NSNumber *adSizeType in adSizes) {
        ...
        adView = [[GNAdView alloc] initWithAdSizeType:adSize appID:_zoneIDTextField.text];
...
- (void)showAdButtonPushed:(id)sender {
    for (GNAdView *adView in _adViews) {
        [adView stopAdLoop];
        adView.delegate = nil;
...
- (void)adView:(GNAdView *)adView didFailReceiveAdWithError:(NSError *)error {
    NSLog(@"didFailReceiveAdWithError: %@", error);
}
